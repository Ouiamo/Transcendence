...
function StatBox({ icon, label, value }: { icon: JSX.Element, label: string, value: number | string }) {
    return (
        <div className="min-w-[300px] max-w-[300px] bg-[#120d1d]/40 border border-[#c44cff]  rounded-[12px] flex flex-row gap-[20px] items-center ">
...
function GameCard({ icon, title, description, onClick }: { icon: JSX.Element, title: string, description: string, onClick: () => void }) {
  ...
  bg-[#120d1d]/60 border border-[#c44cff] rounded-[12px]
  flex-1 min-w-[300px] max-w-[300px]
  transition-all duration-300 ease-out
  hover:shadow-[0_0_10px_rgba(196,76,255,0.6),0_0_20px_rgba(196,76,255,0.4),0_0_40px_rgba(196,76,255,0.3)]
  hover:border-[#ff44ff]
  hover:-translate-y-1
">
                ...
                <button
                    onClick={onClick}
                    className=" mt-[20px] min-w-[200px] max-w-[280px] min-h-[38px] max-h-[80px] py-[3px] px-[6px] mt-[4px] rounded-full bg-gradient-to-r from-[#ff44ff] to-[#ff99ff]   text-sm uppercase tracking-widest transition-all duration-300 outline-none border-none shadow-[0_0_15px_rgba(255,68,255,0.4)] hover:shadow-[0_0_25px_rgba(255,68,255,0.7)] hover:scale-[1.02] active:scale-[0.98]"
                >
                    Play Now
                </button>
...
export function Grid({ listfriends, goto, goto_ia }: gridinter) {
    ...
    return (
        < div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 w-full  mx-auto space-y-[12px] bg-[#0b0618] min-h-screen text-white   " >
